...
final class OnboardingCoordinator {
    ...
    func goBack() {
        guard canGoBack, let prev = OnboardingRoute(rawValue: route.rawValue - 1) else { return }
        FDS.haptic(.light)
        withAnimation(FDS.Spring.page) { route = prev }
    }
    ...
    func toggleGoal(_ goal: OnboardingFitnessGoal) {
        FDS.selectionHaptic()
        if let idx = profile.fitnessGoals.firstIndex(of: goal) {
            profile.fitnessGoals.remove(at: idx)
        } else {
    ...
    func toggleWorkout(_ workout: OnboardingWorkoutType) {
        FDS.selectionHaptic()
        if let idx = profile.preferredWorkouts.firstIndex(of: workout) {
            profile.preferredWorkouts.remove(at: idx)
        } else {
    ...
    private func fetchHealthData() async {
        ...
        healthProfile = prof
        healthSnapshot = snap
        if let h = prof?.heightCm { profile.heightCm = h }
        if let w = prof?.weightKg { profile.weightKg = w }
    }
