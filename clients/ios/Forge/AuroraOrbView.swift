...
struct AuroraOrbView: View {
    ...
    var body: some View {
        ZStack {
            AuroraWaveCanvas(accent: mood.accentColor, amplitude: effectiveAmplitude)
            coreOrb
            pulsingRing
            outerHalo
            shimmerCore
...
private struct AuroraWaveCanvas: View {
    ...
    private func drawWaves(context: GraphicsContext, size: CGSize, time: Double) {
        ...
        for (index, config) in Self.configs.enumerated() {
            ...
            var path = Path()
            for step in 0...32 {
                let angle = Double(step) / 32.0 * .pi * 2
            ...
            path.closeSubpath()
            context.stroke(path, with: .color(accent.opacity(config.opacity)), lineWidth: 1.5)
        }
    }
}
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { timeline in
            ...
            Circle()
                ...
                .offset(x: CGFloat(cos(angle) * (baseRadius + scatter)), y: CGFloat(sin(angle) * (baseRadius + scatter)))
        }
    }
}