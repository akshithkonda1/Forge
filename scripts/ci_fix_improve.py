...
def main() -> int:
    ...
    if not API_DIR.is_dir():
        check("backend/api exists", False, str(API_DIR))
    check("backend/api exists", True)

    if not compileall.compile_dir(str(API_DIR), quiet=1):
        check("Python compileall", False, "syntax errors in backend/api")
    check("Python compileall", True)

    run([sys.executable, str(ROOT / "scripts" / "package_lambda.py")])
    ...
    with zipfile.ZipFile(ZIP_PATH) as archive:
        ...
        for required in ("handler.py", "auth.py", "routes/__init__.py"):
