...
| P0 | SimRunner evaluates the **real** engine (100%-context/isometric ship/hold gate) | 0.5, 5a, 7 | **actually high, not low** | **actually blocks on §7's module decomposition** | **Verified 2026-09-19: still a real gap, but this row's risk/dep columns are wrong.** `ai/simrunner/aria_simrunner/aria_engine.py`'s own docstring confirms it's a stub that never calls production `services/aria_engine.py`. The reason isn't an oversight: "SimRunner stays stdlib-only and must not import the Lambda package" is stated as a deliberate rule in 4 separate files (`dummy_orchestrator.py`, `web_research.py`, `prompts.py`, `data_generator.py`), not just this one docstring. `services/aria_engine.generate_response` lives inside `backend/infra/lambda/services/` — the Lambda package by definition — so calling it from SimRunner means either breaking a rule stated four times on purpose, or first doing the §7 module-decomposition work (extracting the pure interpreter/signal logic into a package both sides can import) as a prerequisite, not a parallel nice-to-have. This needs an explicit decision, not an implementation. |
