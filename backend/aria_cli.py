...
--------
    ...
    python backend/aria_cli.py --profile depleted -m "should I train today?"

    # A primed athlete, voice mode (prose only, no card)
    python backend/aria_cli.py -p primed -m "how's my recovery?" --voice

    # Your own context (the /ai/chat request body shape), from a file or stdin
    python backend/aria_cli.py -m "how did I sleep?" --context-file ctx.json
    ...
    python backend/aria_cli.py -p mixed -m "what should I do today?" --live

    # Redact a domain, interactive session, raw JSON, list the sample users
    python backend/aria_cli.py -p depleted -m "..." --deny sleep,training
    python backend/aria_cli.py -p primed --repl
    python backend/aria_cli.py -p mixed -m "..." --json
    python backend/aria_cli.py --list-profiles
"""
...
# The engine lives in the deployed Lambda package; import it the way dev_server does.
