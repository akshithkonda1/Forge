...
def _tool_get_health_snapshot(user_id: str, _input: dict[str, Any]) -> dict[str, Any]:
    ...
    if readiness_item:
        ...
        if sleep_items:
            ...
            readiness_data = resolve(None, default_readiness, empty_readiness)
...
def _tool_get_sleep_analysis(user_id: str, tool_input: dict[str, Any]) -> dict[str, Any]:
    ...
    sleep_records = (
        ...
        if sleep_items
        else resolve(None, lambda: default_sleep()[:days], list)
    )
...
def _tool_get_personal_records(user_id: str, _input: dict[str, Any]) -> dict[str, Any]:
    ...
    if computed_prs:
        return {"personalRecords": sorted(computed_prs, key=lambda x: x["exercise"])}
    return {"personalRecords": resolve(None, default_personal_records, list)}
...
def _tool_compute_readiness_score(user_id: str, _input: dict[str, Any]) -> dict[str, Any]:
    ...
    recent_sleep = (
        [_strip_keys(i) for i in sleep_items]
        if sleep_items
        else resolve(None, lambda: default_sleep()[:7], list)
    )
...
def _get_connections(user_id: str) -> list[dict[str, Any]]:
    ...
    if not connections:
        connections = resolve(None, default_connections, list)
