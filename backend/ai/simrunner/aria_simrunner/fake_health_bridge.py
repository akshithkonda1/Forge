...
def context_from_pack_day(day: Any, *, persona: str | None = None) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        readiness_7d_avg=_opt(get("readiness7dAvg")) or 0.0,
        hrv_7d_trend=str(get("hrvTrend") or "stable"),
        hrv_7d_avg=_opt(get("hrv7dAvg")) or 0.0,
        target_sleep_hours=8.0,
        target_wake_hour=7.0,
        is_overtrained=bool(get("isOvertrained") or False),
        is_sleep_deprived=float(get("sleepDebtHours") or 0.0) > 5.0,
        ...
        has_sleep_stages=bool(night.get("deepMinutes") or night.get("remMinutes")),
        ...
        hrv_days_available_7d=7 if today.hrv is not None else 0,
        sleep_nights_available_7d=7 if sleep_h is not None else 0,
        missing_fields=[],
