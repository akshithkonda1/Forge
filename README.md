...
[Overview](#-overview) · [Architecture](#-architecture) · [Features](#-features) · [Getting Started](#-getting-started) · [Integrations](#-integrations) · [AI Coach](#-ai-coach) · [SimRunner](#-aria-simrunner--offline-evaluation-harness) · [Roadmap](#-roadmap) · [Contributing](#-contributing)
...
## 🧪 ARIA SimRunner — Offline Evaluation Harness

ARIA gives safety-critical advice ("your readiness is 42 — don't train hard today"),
so it needs more than spot-checks. **SimRunner** is a fully **offline, deterministic**
evaluation harness that stress-tests ARIA's prompt → context → response pipeline and
turns the results into a defensible **ship / hold** decision — before anything reaches
a user. It lives in [`backend/simrunner/`](backend/simrunner/README.md) and is
**stdlib-only** (Python 3.10+, nothing to install).

### How It Works

```
20 behavioral archetypes (+ the full AWS Bedrock catalog)
    ↓
Generate a seeded 30-day health stream per persona
    ↓
Build ARIAContext snapshots → run the tier's adversarial queries
    ↓
Grade every response across 6 dimensions (deterministic, no LLM-as-judge)
    ↓
Triage failures by severity → SHIP / HOLD verdict
    ↓
Reports + regression gate against committed golden baselines
```

### What it tests

- **20 difficulty-graded archetypes** (Tier 1 "Compliant Athlete" → Tier 5
  "System Gamer" / "Genuinely Ambiguous Signal"), plus **any model in the full AWS
  Bedrock catalog** via a deterministically derived persona.
- **Six scoring dimensions** — context utilization, directional correctness,
  chronotype alignment, actionability, epistemic honesty, tone compliance.
- **Mission-critical triage** — every failure is ranked (`mission_critical` → `high`
  → `medium` → `can_wait`); a single safety violation (e.g. recommending high
  intensity at readiness < 50) forces a **HOLD**, with the offending turn spelled out
  as **what / how / when**.
- **Statistical confidence** — `--seeds N` reports `mean ± stdev` and a stability flag
  instead of a single point estimate.
- **Regression gate** — committed golden baselines + a CI step that fails the build on
  any composite regression or newly-introduced mission-critical failure.
- **Real-model grading (opt-in)** — `USE_REAL_API=true` calls a real Bedrock model via
  the Converse API; without creds it falls back to the deterministic stub, no crash.

### Quick start

```bash
# Full suite — all 20 archetypes, with the ship/hold verdict per model
python -m backend.simrunner --all

# Multi-seed statistical confidence
python -m backend.simrunner --all --seeds 5

# Regression gate against the committed baselines (what CI runs)
SIMRUNNER_TODAY=2026-01-15 python -m backend.simrunner --all --gate
```

> **No real API calls, no HealthKit, no tokens by default** — same config + same
> model = identical output, every run. Full docs:
> [`backend/simrunner/README.md`](backend/simrunner/README.md).
...
- [x] [ARIA SimRunner](backend/simrunner/README.md) — offline deterministic eval harness (20 archetypes, 6-dimension scoring, mission-critical ship/hold verdict, regression gate)
...
- [ ] Continuous ARIA evaluation in CI (real-Bedrock grading, prompt A/B, expanded SimRunner archetypes)
