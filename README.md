...
**Forge pulls that picture together** from Apple Health and your phone. One home. One coach. Your baseline, not someone else’s average.

## Meet ARIA

ARIA is the heart of Forge. She’s an **AI lifestyle coach** — a companion who notices patterns in your life and helps you keep habits. She talks like a coach: short, structured, human. She remembers you over time, not just the last message. She is not a chatbot bolted onto a dashboard.
...
Forge is a **strong local iOS app in active development**. The phone experience is the real product.

It is **not on the App Store**, and there is **no public TestFlight** yet. Friends and testers need a local build.
...
This is a solo indie project. I’m still building it. Real software — not a landing-page mock, and not a shipped consumer product yet.
...
iOS is canonical. The Xcode project targets **iOS 27** and **watchOS 27**. `ForgeCore`’s package floor is iOS 18 / watchOS 11 so `swift test` can run on CI. There is also a Next.js web client and a shared Python backend. Deeper plans: [`FORGE_ARIA_BUILD_PLAN.md`](FORGE_ARIA_BUILD_PLAN.md), [`backend/ai/simrunner/README.md`](backend/ai/simrunner/README.md).
...
If ⌘R crashes the Simulator itself (DeviceHub, on Xcode 27) instead of launching the app, that's an Apple host-process crash, not a Forge bug — see [`ForgeSwift/SIMULATORS.md`](ForgeSwift/SIMULATORS.md#devicehub-xcode-27) for the two-command recovery.
